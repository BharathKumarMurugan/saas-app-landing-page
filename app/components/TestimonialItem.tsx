import { testimonialsObject } from "@/constants/data";
...
const TestimonialItem: React.FC<TestimonialItemProps> = ({ testimonial, containerClassName }) => {
  const { name, role, avatarUrl, comment } = testimonial;
  return (
    <div
      className={clsx(
    ...
    >
      ...
      <div className="flex items-center max-xl:-mr-8">
        <div className="mr-4 size-20 shrink-0 rounded-half border-2 border-s2 p-1.5">
          <Image src={avatarUrl} width={100} height={100} alt={name} className="size-full object-cover" />
        </div>
        <div>
